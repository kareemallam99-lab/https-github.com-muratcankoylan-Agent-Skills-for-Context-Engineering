---
name: memory-systems
description: This skill should be used when the user asks to "build persistent agent memory", "choose a memory framework", "implement cross-session state", "compare Mem0 vs Zep vs Letta", "design memory architecture", or discusses agent memory layers, knowledge graphs, vector stores, temporal memory, or memory consolidation. Provides framework selection guidance with benchmark comparisons.
---
...
Select a framework based on the dominant retrieval pattern the agent requires.
...
| **Cognee** | Multi-layer semantic graph via customizable ECL pipeline | Evolving agent memory, multi-hop reasoning | Heavier ingest-time processing |
...
Choose Zep/Graphiti when the agent needs bi-temporal modeling (tracking both when events occurred and when they were ingested) because its three-tier knowledge graph excels at temporal queries. Choose Mem0 when the priority is fast time-to-production with managed infrastructure. Choose Letta when the agent needs deep self-introspection. Choose Cognee when the agent must build dense multi-layer semantic graphs with customizable retrieval across 14 search modes.
...
| Zep (Temporal KG) | 94.8% | — | Mid-range | 2.58s |
| Letta (filesystem) | — | 74.0% | — | — |
| Mem0 | — | 68.5% | Lowest | — |
| MemGPT | 93.4% | — | — | Variable |

Key takeaways: Zep achieves up to 18.5% accuracy improvement on LongMemEval while reducing latency by 90%. Cognee outperformed competitors on HotPotQA multi-hop reasoning. Letta's filesystem agents achieved 74% on LoCoMo using basic file operations, confirming that reliable retrieval beats tool sophistication.

### Memory Layers (Decision Points)

Pick the shallowest memory layer that satisfies the persistence requirement.
...
| **Temporal KG** | Cross-session + history | Graph with validity intervals | Facts that change over time, time-travel queries |

### Retrieval Strategies

Match the retrieval strategy to the query shape.
...
Run consolidation periodically to prevent unbounded growth. **Invalidate but do not discard** — preserving history matters for temporal queries. Trigger consolidation on memory count thresholds, degraded retrieval quality, or scheduled intervals.
...
1. **Prototype**: Use file-system memory. Store facts as structured JSON with timestamps. Validates agent behavior before committing to infrastructure.
2. **Scale**: Move to Mem0 or a vector store when the agent needs semantic search and multi-tenant isolation.
3. **Complex reasoning**: Add Zep/Graphiti when the agent needs relationship traversal, temporal validity, or cross-session synthesis.
4. **Full control**: Use Letta or Cognee when the agent must self-manage its own memory with deep introspection.

### Integration with Context

Load memories just-in-time rather than preloading everything. Place retrieved memories in attention-favored positions (beginning or end of context) to maximize their influence on generation.
...
5. Design for retrieval failure: always have a fallback when lookup returns nothing
6. Consider privacy implications of persistent memory (retention policies, deletion rights)
7. Benchmark against LoCoMo or LongMemEval before and after changes
8. Monitor memory growth and retrieval latency in production

## Gotchas

1. **Stuffing everything into context**: Loading all available memories is expensive and degrades attention quality. Use just-in-time retrieval with relevance filtering instead.
...
5. **Embedding model mismatch**: Writing memories with one embedding model and reading with another produces poor retrieval. Pin a single embedding model for each memory store and re-embed all entries if the model changes.

6. **Graph schema rigidity**: Over-structured graph schemas break when the domain evolves. Prefer generic relation types and flexible property bags.

7. **Stale memory poisoning**: Old memories that contradict the current state corrupt agent behavior silently. Implement expiry policies or confidence decay so the agent deprioritizes aged facts.

8. **Memory-context mismatch**: Retrieving memories that are topically related but contextually wrong. Include session or domain metadata in memory entries and filter on it during retrieval.
...
- context-fundamentals - Foundation for the context layer that memory feeds into
- multi-agent-patterns - When multiple agents need to share or coordinate memory state

External resources:
- Zep temporal knowledge graph paper (arXiv:2501.13956)
- Mem0 production architecture paper (arXiv:2504.19413)
- Cognee optimized knowledge graph paper (arXiv:2505.24478)
